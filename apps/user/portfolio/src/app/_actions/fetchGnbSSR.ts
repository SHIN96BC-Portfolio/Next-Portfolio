...
export default async function fetchGnbSSR(lang: Locale): Promise<SiteGnb[]> {
  const locale = resolveLocale(lang);

  try {
    const service = serviceContainer.get(SERVICE_KEY.SITE);
    const response = await service.getGnb({ lang: locale });
  ...
  return getPortfolioNavigationMock(locale);
}
