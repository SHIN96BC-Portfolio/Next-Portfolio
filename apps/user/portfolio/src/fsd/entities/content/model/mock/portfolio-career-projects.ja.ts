...
export const portfolioCareerProjectsJa: ResumeProjectConfig[] = [
  {
    ...
    workSections: [
      ...
      },
      {
        title: '3) パフォーマンス最適化',
        items: [
          '**ページ読み込み最適化** — SSG/SSR を状況に応じて組み合わせ、TanStack Query キャッシュで不要な API 重複呼び出し・重複ローディングを排除、不要に繰り返し実行されていた useEffect を整理。最も読み込みに時間がかかっていたページを約1/3に短縮',
    ...
    outcomes: [
      '**デプロイ時間を約50〜60%短縮**、主要ページの読み込みを約1/3に短縮し、開発生産性とユーザー体験を同時に向上',
      'メジャーバージョンアップ・状態管理移行を **サービス停止なしで** 完了し、安定性と最新技術スタックを両立',
    ],
    extraSections: [
      {
        title: 'CI/CD・インフラ再設計',
        body: 'FE デプロイパイプライン・Helm は自ら担当し、CDN・セキュリティ・Pod 運用はインフラチームと役割を分けました。',
        items: [
          '**直接担当** — FE デプロイパイプラインの設計・構築（`pipeline-deploys.yml` オーケストレーター、B2C・BP・ONBP × 4環境 20本以上の体系）、Helm チャートの作成および高度化（topologySpreadConstraints、readinessProbe、HPA）、デプロイ検証ロジック',
          '**インフラチームとの協業** — Akamai CDN、インフラセキュリティ、Pod 運用・モニタリング、サーバーログ分析',
        ],
      },
    ],
    techStack: [
