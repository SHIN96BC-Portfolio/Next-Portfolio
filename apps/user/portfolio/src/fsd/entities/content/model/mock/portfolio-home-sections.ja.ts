...
· **CI/CD パイプライン・K8s インフラ再設計に参画** — マルチサービス（B2C・BP・ONBP）のデプロイパイプラインをサービス・環境別に細分化して再設計、Azure Local ARC 移行・DR フェイルオーバー構成に貢献。ビルド〜デプロイ 30分+ → 12〜15分短縮、ページ読み込みを約1/3に短縮（SSG/SSR 組み合わせ・キャッシュ・useEffect 整理）
