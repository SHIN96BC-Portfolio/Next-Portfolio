...
export const portfolioHomeSectionsJa: HomeSectionGetRes[] = [
  ...
  },
  {
    ...
    isActive: true,
    config: {
      body: `**Next.js** ベースのウェブサービスを専門とする、5年目のフロントエンド開発者です。SI・スタートアップ・旅行プラットフォームなど多様な環境で、新規構築とレガシーの次世代移行の両方を担当してきました。

現在は M社の B2C/B2B 旅行プラットフォームで as-is 運用と次世代再構築を並行し、約300の BP/ONBP サイトを単一 Turborepo モノレポで運用する**マルチテナント構造**を設計・実装しました。Next.js 12→15 メジャーバージョンアップと状態管理の移行をサービス無停止で完了し、**FE デプロイパイプライン・Helm チャートの設計**から、**AI コーディングエージェントがモノレポのアーキテクチャ境界を守るためのコンテキスト・検証ハーネス**まで自ら構築し、チーム標準として運用しています。
...
· **パフォーマンス最適化** — 大容量照会のボトルネックを構造的に診断して数十倍単位で改善し、ページ読み込み・再レンダリングのボトルネックも解消
...
· **決済・ルーティングなど critical バグで不安定だったレガシーサービスを安定化** — 400 件以上のイシューを処理し、決済失敗・異常ルーティングなどの核心障害を解消、サービス信頼性を大幅に改善
...
· **最も遅かったページの読み込みを約1/3に短縮** — SSG/SSR の組み合わせ・TanStack Query キャッシュ・不要な useEffect の整理（Lighthouse モバイル基準）

· **Next.js 12→15 メジャーバージョンアップを主導** — App Router・React 19 対応を含む段階的マイグレーションをサービス無停止で実施

· **テスト・品質体系の構築** — 手作業に頼っていた QA を補完するため、Vitest による単体・統合テストと、リモートデプロイ環境を対象とした Playwright E2E を構成し、実行・レポート用の Testbed を自作。Husky の品質ゲートで共通化・メジャーバージョンアップ時のリグレッションを遮断

· **AI エージェントのコンテキスト・検証ハーネスの構築** — モノレポのアーキテクチャ境界（API 契約・アダプター・HTTP レイヤーの分離、B2C↔ONBP の隔離）を AGENTS.md と path-scoped Rule にエンコードして生成時点で遮断し、エージェントの成果物も Biome・Vitest・Playwright の品質ゲートを通過しなければ反映されない検証ハーネスを構成。Cursor・Claude・Gemini・Codex の共通コンテキストとして標準化し、E2E の失敗を後続作業へ連結`,
    },
  },
  {
    id: 'sec-projects',
      ...
      items: [
        {
          ...
          description: 'M社 旅行プラットフォームの次世代開発・運用',
          isDevRole: true,
        },
        {
          ...
          position: '研究員',
          role: 'Frontend Developer',
          description: '自社サービス（フィリピン コマース・配達、動物病院予約プラットフォーム）開発・運用',
          isDevRole: true,
        },
        {
          id: 'career-er',
