...
이 모노레포의 배포는 **Vercel Git 연동 없이**, GitHub Actions에서 `vercel deploy` CLI로 수행합니다.
...
| Vercel Root Directory | **비움** (`.`) — 서브디렉터리 지정 안 함 |
| Actions `vercel deploy` | **앱 폴더** (`working-directory: apps/...`) |
...
Vercel이 **원격 빌드**할 때 사용하는 install/build 명령입니다.  
이 저장소는 **Root Directory를 비우는 방식(패턴 B)** 이므로, `vercel.json`은 반드시 **앱 폴더 안**에 두고, install/build는 `cd`로 **모노레포 루트**로 올라가 실행합니다.
...
### 2-4. Root Directory와의 관계

| 항목 | `vercel.json` | Vercel 대시보드 | GitHub Actions |
|------|:-------------:|:---------------:|:--------------:|
| Root Directory | ❌ **설정 불가** | **비움** (`.`) | — |
| Install / Build Command | ✅ 여기서 정의 | 입력 안 해도 됨 | — |
| `vercel deploy` 실행 위치 | — | — | **앱 폴더** (`working-directory`) |

**왜 `cd`가 필요한가**

- Vercel Root Directory를 **비우면**, 빌드 컨텍스트의 기준은 **앱 폴더**입니다.
- `pnpm-lock.yaml`과 `@core/*` workspace 패키지는 **모노레포 루트**에 있으므로, install/build 전에 `cd`로 루트로 이동해야 합니다.
- Root Directory에 `apps/user/portfolio`를 넣는 방식(패턴 A)이었다면 install은 루트에서 자동 실행되지만, **이 저장소는 패턴 B를 쓰므로 `vercel.json`에서 직접 `cd`합니다.**

**하지 말아야 할 조합**

```
❌ Vercel Root Directory = apps/user/portfolio
   + Actions working-directory = apps/user/portfolio
   → apps/user/portfolio/apps/user/portfolio 경로 중복

✅ Vercel Root Directory = 비움 (.)
   + Actions working-directory = apps/user/portfolio
...
경로: `.github/workflows/deploy-portfolio.yml`
...
checkout
  ...
  → vercel deploy (앱 폴더에서! working-directory)
  → Summary에 배포 URL 출력
```

**중요:** `vercel deploy`는 **앱 폴더**에서 실행합니다 (`working-directory: apps/user/portfolio`).

Vercel 대시보드 Root Directory는 **비워 둡니다** (`.`). Root Directory에 `apps/user/portfolio`를 **또** 지정하면 경로가 중복되어 `apps/user/portfolio/apps/user/portfolio` 오류가 납니다.

```yaml
# deploy step
working-directory: apps/user/portfolio
...
```bash
./scripts/vercel/setup-portfolio-project.sh
```
...
|------|:-------------:|:---------------:|------|
| Install Command | ✅ | 불필요 | `pnpm install --frozen-lockfile` |
| Build Command | ✅ | 불필요 | `turbo build --filter=...` |
| **Root Directory** | ❌ | **비움** (`.`) | 대시보드에서 서브디렉터리 지정 금지 |
| Include files outside root | ❌ | ✅ | 모노레포 `@core/*`용 |
| Environment Variables | (선택) | ✅ | 보통 대시보드에 등록 |
| Git 연동 | — | ❌ 연결 안 함 | Actions만 사용 |
...
| 항목 | 값 |
|------|-----|
| Root Directory | **비움** (`.`) — `apps/user/portfolio` 등 입력하지 않음 |
| Include source files outside of the Root Directory | **Enabled** |
| Framework Preset | Next.js |

#### Root Directory는 비워 두는 이유

이 저장소는 **패턴 B**를 사용합니다.

| | 패턴 A (사용 안 함) | **패턴 B (현재)** |
|--|-------------------|------------------|
| Vercel Root Directory | `apps/user/portfolio` | **비움** (`.`) |
| Actions `vercel deploy` | repo root | **앱 폴더** (`working-directory`) |

Root Directory에 앱 경로를 넣고 Actions에서도 `working-directory`를 쓰면 경로가 **두 번** 붙어 실패합니다. **둘 중 하나만** 씁니다.

5단계 `vercel link`는 **앱 폴더 안**에서 실행하고, Code directory는 **`.`** 입니다. 대시보드 Root Directory가 `apps/...`로 잡혀 있으면 **비우세요**.

`Include source files outside...`가 꺼져 있으면 `@core/*` workspace 패키지를 빌드 중 찾지 못합니다.
...
- Actions 워크플로는 env를 Vercel에 넘기지 않습니다
- Vercel이 **원격 빌드**할 때 대시보드에 등록된 값을 사용합니다
- preview / production 환경별로 분리 설정 가능

### 7-4. Build & Development Settings

대시보드의 Install / Build Command는 **입력하지 않아도 됩니다.** 2단계 `vercel.json`이 배포 시 사용됩니다.

배포 로그에서 아래처럼 나오면 정상입니다.
...
| target | 동작 | URL |
|--------|------|-----|
| `preview` | `vercel deploy` | `*.vercel.app` 프리뷰 URL |
| `production` | `vercel deploy --prod` | Production 도메인 |
...
| 6 | Vercel Root Directory **비움** 확인 | 대시보드 |
...
워크플로 deploy step에 `working-directory: apps/user/commerce` 추가.
...
# deploy job — working-directory: apps/user/commerce
...
| 파일 | 역할 |
|------|------|
| `apps/user/portfolio/vercel.json` | Vercel 원격 빌드 install/build 명령 |
| `.github/workflows/deploy-portfolio.yml` | portfolio lint + deploy 워크플로 |
| `.github/workflows/deploy-master-admin.yml` | master-admin lint + deploy 워크플로 |
| `scripts/vercel/setup-project.sh` | Vercel 프로젝트 최초 생성 (범용) |
| `scripts/vercel/setup-portfolio-project.sh` | portfolio용 래퍼 스크립트 |
| `scripts/vercel/setup-master-admin-project.sh` | master-admin용 래퍼 스크립트 |
| `biome.json` | `lineEnding: "lf"` (CI format 일치) |
...
| `apps/user/portfolio/apps/user/portfolio` does not exist | Root Directory + `working-directory` **둘 다** 설정됨 | Root Directory **비우기** + Actions는 앱 폴더에서 deploy |
| Biome format errors (`␍`) | LF/CRLF 불일치 | `biome.json` `lineEnding: "lf"` + `.gitattributes` |
| `@core/*` 모듈 not found | 모노레포 루트 파일 미포함 | Vercel **Include files outside Root Directory** 활성화 |
| Git push마다 Vercel 자동 배포 | Vercel Git 연결됨 | Git Integration 제거 |
| 로컬 lint 통과, CI만 실패 | 줄바꿈 또는 캐시 차이 | `pnpm biome check .` 로컬 실행 후 LF 정규화 |
| `vercel deploy` 성공했는데 빌드 실패 | Vercel 원격 빌드 오류 | Vercel 대시보드 → Deployments → Build Logs 확인 |
...
□ 7단계: Vercel Root Directory **비움** (.) 확인
□ Vercel Include files outside root = Enabled
...
□ 5단계: ./scripts/vercel/setup-master-admin-project.sh
    → Code directory: apps/admin/master-admin
...
| `working-directory: apps/user/portfolio` | `working-directory: apps/admin/master-admin` |
