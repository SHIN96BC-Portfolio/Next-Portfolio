...
echo ""
echo "앱 폴더에서 vercel link 실행 (Root Directory 비움 방식)"
echo ""
...
echo ""
echo "== Vercel 대시보드 확인 =="
echo "  - Root Directory: 비움 (.) — 서브디렉터리 지정하지 않음"
echo "  - Include source files outside of the Root Directory: Enabled (모노레포 @core/*)"
echo "  - Git Integration: 연결하지 않음"
echo ""
echo "GitHub Actions는 working-directory=${APP_DIR} 에서 vercel pull → vercel build → vercel deploy --prebuilt 합니다."
